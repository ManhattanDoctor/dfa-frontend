...
import { PermissionService, RouterService } from '@core/service';
...
export class CoinMenu extends ListItems<IListItem> {
    ...
    constructor(language: LanguageService, transport: Transport, private router: RouterService, permission: PermissionService) {
        super(language);
        ...
        item.checkEnabled = (item, coin) => CoinUtil.isCanEdit(coin, permission.resources, false);

        item = this.add(new MenuItem('company.company', CoinMenu.COMPANY, 'fa fa-building me-2'));
        item.action = (item, coin) => transport.send(new CompanyOpenCommand({ id: coin.id, isBriefly: true }));
        ...
        item.action = (item, coin) => transport.send(new CoinSubmitCommand());
        item.checkEnabled = (item, coin) => CoinUtil.isCanSubmit(coin, permission.resources, false);

        item = this.add(new MenuItem('coin.activate.activate', CoinMenu.VERIFY, 'fa fa-check me-2'));
        item.action = (item, coin) => transport.send(new CoinActivateCommand());
        item.checkEnabled = (item, coin) => CoinUtil.isCanActivate(coin, permission.resources, false);
